          ...
          <div className="flex-1 px-4 sm:px-6 md:px-8 pt-6">
            ...
            <div className="p-4 md:p-5 flex gap-3 items-center bg-[#F4F7FE] rounded-[16px] mb-6">
              ...
              <p className="text-[14px] sm:text-[15px] md:text-[16px] text-slate-700 leading-relaxed pr-2 font-medium">
                You have <span className="text-blue-600 font-bold">{meetings.length}</span> pending meetings.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4 md:gap-6">
              ...
              {meetings.map((meeting: any) => {
                ...
                return (
                  <div key={meeting.id} className="relative rounded-[16px] border p-4 sm:p-5 md:p-6 overflow-hidden group hover:shadow-md transition-shadow cursor-pointer bg-[#F4F7FE] border-blue-100">
                    <div className="absolute left-0 top-3 bottom-3 w-1.5 md:w-2 rounded-r-md bg-blue-600" />
                    
                    <div className="pl-3 sm:pl-4 md:pl-5">
                      {/* Top Row */}
                      <div className="flex justify-between items-start mb-3 md:mb-4">
                        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                          <div className="w-11 h-11 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full text-slate-800 flex items-center justify-center font-bold text-[15px] md:text-[17px] shrink-0 bg-blue-200/50">
                            {initials}
                          </div>
                          <div className="min-w-0 truncate">
                            <h3 className="font-bold text-[16px] md:text-[18px] text-slate-900 truncate">{otherPerson?.name || "Unknown"}</h3>
                          </div>
                        </div>
                        {/* Badge */}
                        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full shrink-0 bg-blue-100 text-blue-700 border border-blue-200/50">
                          <span className="text-[12px] md:text-[13px] font-semibold">{meeting.status}</span>
                        </div>
                      </div>
                      ...
                      <div>
                        <p className="text-[14px] md:text-[15px] text-slate-700 font-medium mb-1 truncate">Meeting Notes</p>
              ...
              {meetings.length === 0 && (
                <div className="text-center py-12">
                  <div className="w-16 h-16 rounded-full bg-blue-50 text-blue-200 mx-auto flex items-center justify-center mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 mb-2">No meetings</h3>
                  <p className="text-slate-500">You don't have any scheduled meetings yet.</p>
                </div>
              )}
            </div>
          </div>
